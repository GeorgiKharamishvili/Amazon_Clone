...
const Cart = () => {
  ...
  useEffect(() => {
    let price = 0;
    products.map((item) => {
      price += item.price ? item.price : item.newPrice  * item.quantity;
      return price;
    });
      ...
      {products.length > 0 ? (
                ...
                >
                  <div className="w-full flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="w-full flex flex-col gap-2 xl:gap-1">
                      ...
                      <p className="xl:pr-10 line-clamp-2 text-sm">
                        {item.description}
                      </p>
                      <p className="text-base">
                        ...
                        {/* <span className="font-semibold">${ Number(item.price).toFixed(2) ? Number(item.newPrice).toFixed(2) : item.price }</span> */}
                        <span className="font-semibold">${ item.price ? item.price: item.newPrice}</span>
                    ...
                    <div className="w-full md:w-24">
                      <p className="text-lg xl:w-24 font-titleFont font-semibold">
                        ...
                        ${ Number( item.price ? item.price: item.newPrice * item.quantity).toFixed(2)}
                      </p>
                    </div>
                    {/* ====================== Right End here ============================= */}
                  </div>
          ...
          <div className="col-span-5 md:col-span-3 lg:col-span-2 xl:col-span-1 bg-white h-52 flex items-center p-4">
            <div>
              ...
              <div>
                ...
                Total: <span className="text-lg font-bold">${Number(totalAmt).toFixed(2)}</span>
        ...
        >
          ...
          <div className="w-96 p-4 bg-white flex  flex-col items-center rounded-md shadow-lg">
            ...
            <Link to="/">
              <button className="mt-6 bg-yellow-300 rounded-md cursor-pointer hover:bg-yellow-200 active:bg-yellow-100 px-6 py-3 font-titleFont font-semibold text-lg">
                Continue Shopping
              </button>
            </Link>
          </div>
